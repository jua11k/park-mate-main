...
export const parkingPlans = parkMateSchema.table("parking_plans", {
  ...
  gracePeriodMin: integer("grace_period_min").default(0),
  differentialRatePrice: decimal("differential_rate_price", { precision: 12, scale: 2 }),
  differentialRateAfterHr: integer("differential_rate_after_hr"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
  deletedAt: timestamp("deleted_at"),
}, (table) => ({
